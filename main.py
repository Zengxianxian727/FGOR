...
print(args)
os.makedirs('ShiftedSoftmax', exist_ok=True)
...
args.save_result_path = 'ShiftedSoftmax/trained'
...
class ShiftedSoftmax(torch.nn.Module):
    def __init__(self, t = 0):
        super(ShiftedSoftmax, self).__init__()
        self.t = t
    ...
    def forward(self, features, labels):
        ...
        for i in range(batch_size):
            ...
            p = torch.exp(feat) / torch.sum(torch.exp(feature))
            loss = torch.abs(torch.log(p + self.t)) if p>0.4 else torch.abs(torch.log(p))
            loss_crossentropy = loss_crossentropy + loss 
...
def main():
    ...
    criterion = ShiftedSoftmax(t=args.transformed)
    milestones = [100, 200]
